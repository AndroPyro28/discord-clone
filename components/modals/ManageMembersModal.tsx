      ...
      <DialogContent className="bg-white text-black overflow-hidden">
        ...
        <ScrollArea className="mt-8 max-h-[420px] pr-6">
          {server?.members?.map(({ user, id: memberId, role }) => {
            ...
            const isNotAdmin = server.userId !== user.id && loadingId !== memberId;
                                ...
                                <DropdownMenuPortal >
                                    <DropdownMenuSubContent>
                                        <DropdownMenuItem onClick={() => onRoleChange(memberId, 'GUEST')}>
                                            ...
                                            GUEST
                                            {role === 'GUEST' && <Check className="w-4 h-4 ml-auto" />}
                                        </DropdownMenuItem>

                                        <DropdownMenuItem onClick={() => onRoleChange(memberId, 'MODERATOR')}>
                                            <ShieldCheck className="h-4 w-4 mr-2"/>
                                            MODERATOR
                                            {role === 'MODERATOR' && <Check className="w-4 h-4 ml-auto" />}
                                        </DropdownMenuItem>
